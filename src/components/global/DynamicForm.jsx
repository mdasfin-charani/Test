  ...
  const handleSubmit = async (e) => {
    ...
    if (status === 'Pending') {
      ...
      if (isHubAdmin && title.includes('Store')) {
        import('@/utils/emailMock.js').then(({ sendStoreRequestToSuperEmail }) => {
      ...
      if (isSuperAdmin) {
        const { sendDirectCreationEmail } = await import('@/utils/emailMock.js');
...
const SingleEntryForm = ({ config }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (status === 'Pending') {
      ...
      // TRIGGER EMAIL FOR SUPER ADMIN DIRECT CREATION
      if (isSuperAdmin) {
        import('@/utils/emailMock.js').then(({ sendDirectCreationEmail }) => {
